...
namespace Assets.FantasyHeroes.Scripts
{
    ...
    public class CharacterEditor : MonoBehaviour
    {
        ...
        public void Reset()
        {
            ...
            Dummy.Helmet = Dummy.Back = Dummy.PrimaryMeleeWeapon = Dummy.SecondaryMeleeWeapon = Dummy.Shield = Dummy.BowArrow = Dummy.BowLimb = Dummy.BowRiser = null;
            Refresh();
            InitializeDropdowns();
        }
        ...
        private void InitializeDropdowns()
        {
            if (Grid.activeSelf)
            {
                ...
                InitializeDropdown(HairDropdown, SpriteCollection.HairShort.Any(i => i.Sprite == Dummy.Hair) ? SpriteCollection.HairShort : SpriteCollection.Hair, Dummy.Hair, entry => Dummy.Hair = GetSprite(entry));
                ...
                InitializeDropdown(HelmetDropdown, SpriteCollection.Helmet, Dummy.Helmet, entry => Dummy.Helmet = GetSprite(entry));
        ...
        private void InitializeDropdown(Dropdown dropdown, List<SpriteGroupEntry> entries, Sprite equipped, Action<SpriteGroupEntry> onEntrySelected)
        {
            ...
            UnityAction<int> onValueChanged = index => { onEntrySelected(list[index]); Refresh(); };
        ...
        {
            if (SpriteCollection.Hair.Any(i => i.Sprite == Dummy.Hair) && Dummy.Helmet != null)
            {
                Dummy.Hair = SpriteCollection.HairShort.Select(i => i.Sprite).SingleOrDefault(i => i.name == Dummy.Hair.name);
            }
            else if (SpriteCollection.HairShort.Any(i => i.Sprite == Dummy.Hair) && Dummy.Helmet == null)
            {
                Dummy.Hair = SpriteCollection.Hair.Select(i => i.Sprite).SingleOrDefault(i => i.name == Dummy.Hair.name);
            }
